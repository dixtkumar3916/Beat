...
export class BackupManager {
  ...
  private static async restoreRoom(
    ...
    try {
      ...
      // Concurrently validate all audio sources in local storage (no limit on concurrency)
  ...
  static async backupState(): Promise<void> {
    try {
      ...
      // Upload to local storage using the utility function
      const filename = await saveBackup(backupData);
  ...
  /**
   * Restore server state from the latest backup in local storage
   */
  static async restoreState(): Promise<boolean> {
    try {
      ...
      const latestBackup = await loadLatestBackup();
      ...
      const parseResult = ServerBackupSchema.safeParse(latestBackup.data);
  ...
  static async cleanupOldBackups(keepCount: number = 5): Promise<void> {
    ...
    console.log(`✅ Backup cleanup completed. Kept ${keepCount} most recent backups.`);
  }

  /**
   * Clean up orphaned rooms that exist in storage but not in server memory
   */
  static async cleanupOrphanedRooms(): Promise<void> {
    try {
      ...
      cleanupOrphanedRooms(activeRooms, true);
    } catch (error) {
