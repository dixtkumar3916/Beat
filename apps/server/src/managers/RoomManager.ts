...
import { deleteRoomFiles } from "../lib/localStorage";
...
export class RoomManager {
   ...
   */
  processClientLoadedAudioSource(clientId: string, server: Server<WSData>): void {
    if (!this.pendingPlay) {
      console.warn(
   ...
   */
  private executeScheduledPlay(server: Server<WSData>): void {
    if (!this.pendingPlay) {
      return;
    }
   ...
   */
  reorderClients(clientId: string, server: Server<WSData>): ClientDataType[] {
    const clients = this.getClients();
   ...
   */
  moveClient(clientId: string, position: PositionType, server: Server<WSData>): void {
    const client = this.clientData.get(clientId);
    if (!client) return;
   ...
   */
  updateListeningSource(position: PositionType, server: Server<WSData>): void {
   ...
   */
  setGlobalVolume(volume: number, server: Server<WSData>): void {
  ...
  async cleanup(): Promise<void> {
    ...
    try {
      const result = deleteRoomFiles(this.roomId);
      console.log(
        `✅ Room ${this.roomId} local files deleted: ${result.deletedCount}`
      );
    } catch (error) {
